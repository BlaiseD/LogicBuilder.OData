...
namespace LogicBuilder.AspNetCore.OData
{
    public static class LinqExtensions
    {
            ...
            if (options.OrderBy == null)
            {
                return Expression.Call
                    (
                        typeof(Queryable),
                        "Take",
                        new[] { typeof(T) }, expression, Expression.Constant(int.Parse(options.Top.RawValue))
                    );
            }
        ...
        public static List<string> GetSelects(this SelectExpandQueryOption clause)
        {
            ...
            //PathSelectItem path = null;
            return clause.SelectExpandClause.SelectedItems.OfType<PathSelectItem>().Aggregate(new List<string>(), (list, next) =>
            {
                string path = next.SelectedPath.FirstSegment.Identifier;//Only first segment is necessary because of the new syntax $expand=Builder($expand=City) vs $expand=Builder/City
        ...
        public static List<string> GetIncludes(this SelectExpandQueryOption clause)
        {
            ...
            return clause.SelectExpandClause.SelectedItems.OfType<ExpandedNavigationSelectItem>().Aggregate(new List<string>(), (list, next) =>
            {
        ...
        private static List<string> GetNestedIncludes(this ExpandedNavigationSelectItem node)
        {
            ...
            return node.SelectAndExpand.SelectedItems.OfType<ExpandedNavigationSelectItem>().Aggregate(new List<string>(), (list, next) =>
            {
                string path = next.PathToNavigationProperty.FirstSegment.Identifier;//Only first segment is necessary because of the new syntax $expand=Builder($expand=City) vs $expand=Builder/City
        ...
        private static Expression<Func<TSource, object>> BuildSelectorExpression<TSource>(string fullName, string parameterName = "i")
            => (Expression<Func<TSource, object>>)BuildSelectorExpression(typeof(TSource), fullName);
